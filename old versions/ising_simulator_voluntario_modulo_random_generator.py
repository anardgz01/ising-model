...
def simulate(sorted : bool, T : float, N : int, num_Monte_Carlo_steps : int, path : str):
    ...
    def generate_random_values(batch_size=1000000):
        while True:
            random_positions_x = np.random.randint(0, N, size=batch_size, dtype=np.uint8)
            random_positions_y = np.random.randint(0, N, size=batch_size, dtype=np.uint8)
            ksi_values = np.random.uniform(0.,1., size=batch_size)
            for i in range(batch_size):
                yield random_positions_x[i], random_positions_y[i], ksi_values[i]
    
    random_values_generator = generate_random_values()
    measuring_landmark_iterations = Monte_Carlo_step*measuring_landmark
    measure_index = -1
    #Start iterating
    for t in range(1,iterations+1):
        pos_x, pos_y, ksi = next(random_values_generator)
        pos = (pos_x, pos_y)
        probability = p(pos)
